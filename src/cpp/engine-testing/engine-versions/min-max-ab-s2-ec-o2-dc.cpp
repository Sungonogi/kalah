...
// alpha beta and score 2 and early cutoff, getMoves2 and depth Change
class MinMaxABS2ECO2DC: public MinMax {
private:
    // just for tracking
    int depthSum = 0;
    int depthCount= 0;
...
public: 
    ...
    int minMax(BoardPosition &bp, int depth, int alpha, int beta) {
        ...
        for(int i = 0; i < MAX_PIT_SIZE + 1; i++){
            ...
            int tmpScore = minMax(bpCopy, depth + 1 + i / 2, nAlpha, nBeta);
